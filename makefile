.PHONY: help clean update-packages build run-transaction-demo run-otgui test test-netcat-client-only test-netcat-server-only format tag version checkGitClean mermaid bump-patch bump-minor bump-major release

.DEFAULT_GOAL := help

APP_NAME = spmDemosUITools
BUILD_DIR = .build
...
run-transaction-demo: ## Run TransactionDemo
	swift run TransactionDemo

run-otgui: ## Run OTGUIDemo
	swift run OTGUIDemo
